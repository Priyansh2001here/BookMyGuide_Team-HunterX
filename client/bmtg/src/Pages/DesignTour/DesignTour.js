...
function DesignTour() {
    ...
    const addSlot = (e) => {
        ...
        div.onclick = function () {
            var timeToDelete = div.innerHTML;
            var idx = slots.indexOf(timeToDelete);
            if (idx > -1) {
                slots.splice(idx, 1);
            }
            console.log(slots);
    ...
    const createTour = () => {
        ...
        console.log(slotTimings, daysArray)

        makePostRequest("http://localhost:8000/guide/package-create", {
            timings: slotTimings,
            title: tourName,
        ...
        }, "application/json").then(() => window.location.replace("/guide/dashboard"))

    };

    return (
        <div className="designTour">
            <PageHeader pageTitle={<>Design your <span style={{color: "#f78383"}}>Tour</span></>}/>
            <form style={{marginTop: "60px"}}>
                ...
                <br/>
                <input type="number" onInput={e => setTourPrice(e.target.value)} value={tourPrice} name="tourPrice"
                       required/>
                <br/>
